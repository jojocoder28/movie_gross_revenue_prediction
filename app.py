...
runtime = st.sidebar.slider("Runtime (in minutes)", min_value=df['runtime'].min(),max_value=df['runtime'].max(),value=df['runtime'].median())
votes = st.sidebar.slider("Votes (in thousands)", min_value=df['votes'].min(),max_value=df['votes'].max(),value=df['votes'].median())
budget = st.sidebar.number_input("Budget (in millions)", min_value=df['budget'].min(),max_value=df['budget'].max(), value=100)
